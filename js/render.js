...
const walls = [
    ...
    { z: -2.1},   // Pared de enfrente
    { x: -5},  // Pared comedor
    { x: 3.2 },   // Pared al comedor
    { y: -2.70},  // Techo
    { y: -0},   // Piso
];
...
function isColliding(inv) {
    const x = inv[12]; // Posición X de la cámara
    const y = inv[13]; // Posición Y de la cámara
    const z = inv[14]; // Posición Z de la cámara

    // Comprobar colisiones con las paredes definidas
    for (const wall of walls) {
        if (wall.z !== undefined) {
            // Comprobar colisión en el eje Z
            if (Math.abs(z - wall.z) < 0.1) { // Usa un margen de 0.1 para detectar colisiones
                return true; // Colisión detectada
            }
        }
        if (wall.x !== undefined) {
            // Comprobar colisión en el eje X
            if (Math.abs(x - wall.x) < 0.1) {
                return true; // Colisión detectada
            }
        }
        if (wall.y !== undefined) {
            // Comprobar colisión en el eje Y
            if (Math.abs(y - wall.y) < 0.1) {
                return true; // Colisión detectada
            }
        }
    }
...
let defaultViewMatrix = [0.9998656161701728, 0.011569183693536668, -0.011614800447222748, 0, -0.015616553342421558, 0.9961703653557925, 0.000852319461906609, 0, -0.052235522544311255, -0.004386838557299924, 1.002323403813419, 0, 1.9083190114204835, 0.8956258328341457, 1.9808436247510566, 0.9999999999999677];
...
async function main() {
    ...
    const defaultViewMatrix = [0.9998656161701728, 0.011569183693536668, -0.011614800447222748, 0, -0.015616553342421558, 0.9961703653557925, 0.000852319461906609, 0, -0.052235522544311255, -0.004386838557299924, 1.002323403813419, 0, 1.9083190114204835, 0.8956258328341457, 1.9808436247510566, 0.9999999999999677];

    const frame = (now) => {
        let inv = invert4(viewMatrix);
        let invNew = invert4(viewMatrix);
    
        // Movimiento basado en las teclas activas
        if (activeKeys.includes("ArrowUp")) invNew = translate4(inv, 0, 0, 0.1); 

        if (activeKeys.includes("ArrowDown")) invNew = translate4(inv, 0, 0, -0.1);

        if (activeKeys.includes("ArrowLeft")) invNew = translate4(inv, -0.03, 0, 0);

        if (activeKeys.includes("ArrowRight")) invNew = translate4(inv, 0.03, 0, 0);

        if (activeKeys.includes("KeyA")) invNew = rotate4(inv, -0.01, 0, 1, 0);

        if (activeKeys.includes("KeyD")) invNew = rotate4(inv, 0.01, 0, 1, 0);

        if (activeKeys.includes("KeyW")) invNew = rotate4(inv, 0.005, 1, 0, 0);

        if (activeKeys.includes("KeyS")) invNew = rotate4(inv, -0.005, 1, 0, 0);
            // Reiniciar a los valores por defecto cuando se presiona "R"
        if (activeKeys.includes("KeyR")) {
            viewMatrix = [...defaultViewMatrix];
            invNew = invert4(viewMatrix);  // Reemplaza la matriz con los valores por defecto
        }
        
        // Verificar colisiones con las nuevas posiciones
